...
const setResults = ({
  ...
  vi.mocked(useQuery).mockReturnValue({
    loading,
    data: { pokemonByType: { pokemon: data, total } },
...
describe("TypePokemon", () => {
  ...
  describe("sorting", () => {
    it("writes the new sort to the URL and drops back to page 1", async () => {
      ...
      expect(window.history.pushState).toHaveBeenCalledWith(null, "", "/type/fire?sort=NAME_DESC");
    });
  });
  ...
  describe("rendering", () => {
    it("shows its own heading", () => {
      ...
      expect(screen.getByRole("heading", { name: "Pokemon of this type" })).toBeInTheDocument();
    });
    ...
    it("keeps the current grid up while the next result set loads", () => {
      vi.mocked(useQuery).mockReturnValue({
        loading: true,
        data: undefined,
        previousData: { pokemonByType: { pokemon, total: 1 } },
      } as unknown as ReturnType<typeof useQuery>);
  ...
  describe("Pokemon links", () => {
    it("carries the page and sort so closing a Pokemon returns to the same list", () => {
      ...
      expect(screen.getByRole("link", { name: "charmander" })).toHaveAttribute(
        "href",
        "/type/fire/pokemon/4?sort=NAME_DESC&page=3",
      );
    });
  });
  ...
  describe("paging", () => {
    it("keeps the current sort when moving to another page", async () => {
      ...
      expect(window.history.pushState).toHaveBeenCalledWith(
        null,
        "",
        "/type/fire?sort=NAME_DESC&page=2",
      );
    });
  });
});
